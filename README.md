...
#### v1.2.0 (2026-06-02)
- **新機能: INI設定ファイルによる自動展開対応**  
  `-Config <パス>` パラメーターでINIファイルを指定して無人実行できるようになりました。  
  GPO・タスクスケジューラから管理者確認なしで全設定を自動適用できます。  
  ネットワーク共有から各PCにローカルコピーして実行する `Deploy-RansomShield.ps1` ラッパースクリプトを同梱。  
  PC名別INI（例: `RansomShield_PC01.ini`）でPC固有の信頼IP・許可アプリを管理できます。  
  適用結果をWindowsイベントログ（Application/EventID 1000）に記録（`WriteEventLog=1` 時）。
...
#### v1.2.0 (2026-06-02)
- **New: INI config file for unattended deployment**  
  Use `-Config <path>` parameter to apply all settings silently from an INI file — no user interaction needed.  
  `Deploy-RansomShield.ps1` wrapper script copies files locally from a network share before execution (handles network unavailability at boot).  
  Per-PC INI files (e.g., `RansomShield_PC01.ini`) allow custom trusted IPs and allowed apps per machine.  
  Results are logged to Windows Event Log (Application / EventID 1000) when `WriteEventLog=1`.
